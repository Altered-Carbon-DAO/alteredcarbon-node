...
acarbd tx wasm store cw721_base.wasm --from validator $TXFLAG
...
acarbd tx wasm instantiate 1 "$INIT" --from creator --label "register username" $TXFLAG
...
# query contract
acarbd q wasm contract-state smart $CONTRACT '{"num_tokens":{}}'
acarbd q wasm contract-state smart $CONTRACT '{"contract_info":{}}'
...
acarbd tx wasm execute $CONTRACT $MINT1 --from creator $TXFLAG
acarbd tx wasm execute $CONTRACT $MINT2 --from creator $TXFLAG
acarbd tx wasm execute $CONTRACT $MINT3 --from creator $TXFLAG
acarbd tx wasm execute $CONTRACT $MINT4 --from creator $TXFLAG
acarbd tx wasm execute $CONTRACT $MINT5 --from creator $TXFLAG

# query contract
acarbd q wasm contract-state smart $CONTRACT '{"owner_of":{"token_id":"1"}}'
acarbd q wasm contract-state smart $CONTRACT '{"num_tokens":{}}'
acarbd q wasm contract-state smart $CONTRACT '{"nft_info":{"token_id":"1"}}'
acarbd q wasm contract-state smart $CONTRACT "{\"tokens\":{\"owner\":\"$INVESTOR\"}}"

# execute an NFT transfer
TRANSFER="{\"transfer_nft\":{\"recipient\":\"$CREATOR\",\"token_id\":\"1\"}}"
acarbd tx wasm execute $CONTRACT $TRANSFER --from investor $TXFLAG
